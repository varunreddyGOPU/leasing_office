...
- [x] **Phase 4 — Resident portal + auto-renewal + maintenance**: cookie-session auth (stdlib scrypt + HMAC tokens), `GET /api/me/lease` computes the renewal offer inside the T-95 window (`min(market asking, rent × 1.05)`), one-click accept (creates the renewal lease) / decline (flags the unit for availability), maintenance tickets with photo upload (local volume, validated types/sizes), office kanban at `/office/maintenance` (X-Admin-Key gated), every transition writes to the `alerts` outbox. Demo login: `resident1@example.com` / `auburn-demo`; admin key default `dev-admin`.
